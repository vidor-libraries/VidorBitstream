...
alt_u32 tmrStart(alt_u32 ms, void (*hook)(void*), void* arg)
{
  alt_u32 ctr;
  int     ret;
  alt_u32 ticks_x_msec = TMR_FREQ / 1000;

  tmrHook = hook;
  if (hook) {
#ifdef ALT_ENHANCED_INTERRUPT_API_PRESENT
    ret = alt_ic_isr_register(TMR_IRQ_CTR_ID, TMR_IRQ, tmrIsr, arg, 0);
#else
    ret = alt_irq_register(TMR_IRQ, arg, tmrIsr);
#endif
    ctr = TMR_CTR_START | TMR_CTR_ITO;
  } else {
#ifdef ALT_ENHANCED_INTERRUPT_API_PRESENT
    ret = alt_ic_irq_disable(TMR_IRQ_CTR_ID, TMR_IRQ);
#else
    ret = alt_irq_register(TMR_IRQ, NULL, NULL);
#endif
    ctr = TMR_CTR_STOP;
  }
  if (ret) {
  ...
  ms = ms * ticks_x_msec;
  /* set timeout period */
  IOWR(TMR_BASE, TMR_REG_TPL, (alt_u16)(ms & 0x0000FFFF));
  IOWR(TMR_BASE, TMR_REG_TPH, (alt_u16)(ms >> 16));

  /* start or stop timer */
  IOWR(TMR_BASE, TMR_REG_CTR, ctr);
