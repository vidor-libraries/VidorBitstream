...
alt_u32 sfErase(alt_u32 mode, alt_u32 adr)
{
    ...
    case 2:
		...
		IOWR_IPTRONIX_QSPI_CONTROLLER2_MEM_OP(QSPI_CSR_BASE,
				IPTRONIX_QSPI_CONTROLLER2_MEM_OP_WRITE_ENABLE_CMD);

		adr >>= 16;
		op_value = (adr << 8) & IPTRONIX_QSPI_CONTROLLER2_MEM_OP_SECTOR_VALUE_MASK;
		op_value |= IPTRONIX_QSPI_CONTROLLER2_MEM_OP_SECTOR_ERASE_CMD;

		/* write sector erase command to QSPI_MEM_OP register to erase sector "sector_number" */
		IOWR_IPTRONIX_QSPI_CONTROLLER2_MEM_OP(QSPI_CSR_BASE, op_value);

		/* check whether erase triggered a illegal erase interrupt  */
		if((IORD_IPTRONIX_QSPI_CONTROLLER2_ISR(QSPI_CSR_BASE) &
			...
			IOWR_IPTRONIX_QSPI_CONTROLLER2_ISR(QSPI_CSR_BASE,
					IPTRONIX_QSPI_CONTROLLER2_ISR_ILLEGAL_ERASE_MASK);
    ...
    case 3:
		...
		op_value = 0;
		op_value |= IPTRONIX_QSPI_CONTROLLER2_MEM_OP_BULK_ERASE_CMD;
...
alt_32 static poll_for_wip(void)
{
	...
	while ((IORD_IPTRONIX_QSPI_CONTROLLER2_STATUS(QSPI_CSR_BASE) &
