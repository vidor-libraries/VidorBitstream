...
alt_u32 sfJedecId(void)
{
  alt_u32 *csr_base = (alt_u32 *)QSPI_CSR_BASE;
  /**/
//  alt_u32 jid[5];
  memcpy(jid, csr_base+8, 5*sizeof(alt_u32));
  return jid[0];
  /**/
  //return csr_base[8];
}
...
#if 0 //defined(SECURITY) && (SECURITY == 1)
/**
...
alt_u32 sfSRErase(alt_u8 reg)
{
	...
	/* write security register erase command to QSPI_MEM_OP register to erase register "reg" */
	op_value = reg<<8;
	op_value |= ALTERA_QSPI_CONTROLLER_MEM_OP_SEC_REG_ERASE_CMD;
	IOWR_ALTERA_QSPI_CONTROLLER_MEM_OP(QSPI_CSR_BASE, op_value);
	...
	if((IORD_ALTERA_QSPI_CONTROLLER_ISR(QSPI_CSR_BASE) &
		...
		IOWR_ALTERA_QSPI_CONTROLLER_ISR(QSPI_CSR_BASE,
		ALTERA_QSPI_CONTROLLER_ISR_ILLEGAL_ERASE_MASK);
		return -1; /* erase failed, register might be protected */
	}
	poll_for_wip();
...
/*	op_value = reg<<8;
	op_value |= ALTERA_QSPI_CONTROLLER_MEM_OP_SEC_REG_ERASE_CMD;
	IOWR_ALTERA_QSPI_CONTROLLER_MEM_OP(QSPI_CSR_BASE, op_value);*/
// TODO come gli passo i byte da scrivere?
// opz 1: Dario mi mappa 768 byte per i registri sicuri
	/* check whether write triggered a illegal write interrupt */
	if ((IORD_ALTERA_QSPI_CONTROLLER_ISR(QSPI_CSR_BASE) &
			ALTERA_QSPI_CONTROLLER_ISR_ILLEGAL_WRITE_MASK) ==
			ALTERA_QSPI_CONTROLLER_ISR_ILLEGAL_WRITE_ACTIVE) {
